...
const Restaurant = () => {
    ...
    useEffect(() => {
        if (currentPosition.lat !== null) {
            initializeMap(); // 위치 정보가 준비되면 지도 초기화
        }
    }, [currentPosition]);
    ...
    const initializeMap = () => {
        if (window.kakao) {
            const center = new window.kakao.maps.LatLng(currentPosition.lat, currentPosition.lon);
            ...
            console.error('카카오 맵 API가 로드되지 않았거나, 위치 정보가 부족합니다.');
        }
    };

    useEffect(() => {
        if (window.kakao) {
            initializeMap(); // 카카오 맵 API가 로드되었을 때만 호출
        } else {
            const script = document.createElement('script');
            script.src = `//dapi.kakao.com/v2/maps/sdk.js?appkey=${process.env.REACT_APP_KAKAO_MAP_KEY}&autoload=true`;
            script.async = true;
            script.onload = () => {
